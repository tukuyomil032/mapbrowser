...
| **コンパニオン Mod** | クライアント側に任意でインストールする Fabric Mod（音声・高解像度対応、Phase 4） |
...
| ID | 要件 | 優先度 |
|----|------|--------|
| F-AUD-01 | コンパニオン Mod 導入時、スクリーン周辺の音声を再生できる | Should |
...
| # | タスク |
|---|--------|
| 1 | Fabric Mod プロジェクトセットアップ |
| 2 | CDP 音声キャプチャ（Node.js 側） |
| 3 | Opus エンコード / デコード実装 |
| 4 | カスタムパケット定義（Java ↔ Fabric Mod） |
| 5 | OpenAL 3D 空間音声再生（Fabric Mod 側） |

### Phase 5 — Velocity 対応・拡張

| # | タスク |
|---|--------|
| 1 | Velocity Plugin Messaging Channel 実装 |
| 2 | Simple Voice Chat API 連携（オプション） |
| 3 | パブリック API の公開（他プラグインから操作可能に） |
| 4 | ドキュメント・Wiki 整備 |
